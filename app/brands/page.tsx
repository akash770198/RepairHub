...
export default function BrandsPage() {
  const { brandsInfo, brands } = site.brands;

  return (
    <main className="flex min-h-screen flex-col">
      <PageBanner data={site.brandsBanner} />

      {/* Brands grid — page-specific layout (home Brands is a logo carousel) */}
        ...
        <div className="page-gutter flex flex-col items-center">
          ...
          <div className="w-full mt-16 grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
            {brands.map((brand) => (
              <div
                key={brand.id}
                className="relative flex aspect-[2/1] items-center justify-center overflow-hidden rounded-md bg-white border border-slate-200 p-4 shadow-sm hover:shadow-md transition-all hover:-translate-y-1 group"
              >
                {brand.logo ? (
                  <Image
                    src={brand.logo}
